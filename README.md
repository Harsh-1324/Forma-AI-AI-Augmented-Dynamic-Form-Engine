...
form whose structure and validation rules live entirely in a MongoDB/Postgres (prisma)
JSON schema.
...
```
forma-ai/
├── server/     # Node.js + LangChain + MongoDB/postgres API
...
```bash
cd server
cp .env.example .env      # fill in postgress/MONGO_URI and gemini_API_KEY
npm install
